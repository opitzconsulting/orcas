...
) materialized view log with primary key, sequence(col_add_ix_2), commit_scn;
...
(
  ...
  col_add_ix_3 number(15) not null,
  primary key (col_add_ix_1)
) materialized view log with primary key, sequence(col_add_ix_2, col_add_ix_3);
...
(
  col_add_ix_1 number(15) not null,
  primary key (col_add_ix_1)
) materialized view log parallel 8;
