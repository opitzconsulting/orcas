    ...
    protected void run() throws Exception {
        ...
        try (Writer lWriter = new OutputStreamWriter(new FileOutputStream(lFile), getParameters().getEncoding())) {
            JdbcConnectionHandler.runWithCallableStatementProvider(getParameters(), pCallableStatementProvider -> {
                String
                    lSql =
                    "select 'grant ' || privilege || ' on ' || user || '.' || table_name || ' to ' || grantee || case when grantable = 'YES' then ' with grant option' end || case when hierarchy = 'YES' then ' with hierarchy option' end || ';' as grant_statement from all_tab_privs_made where not("
