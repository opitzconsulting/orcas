...
begin
  loop
    ...
    (
      ...
      exception
        when others then
          dbms_output.put_line( 'Fehler bei : ' || cur_sql.text || ' Grund: ' || sqlerrm );
      end;
    end loop;
...
begin
  ...
  loop
    raise_application_error( -20000, 'Kompilierungsfehler' );
  end loop;
end;
/
