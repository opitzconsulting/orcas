...
public class GraphForSingleTableDescendants extends GraphForSingleTable
{
  ...
  public GraphForSingleTableDescendants( Table pSingleTable, List<Graph> pParentGraphs, Styles pStyles )
  {
  ...
  public List<Association> getVisibleAssociation( Table pTable, Schema pSchema, boolean pOutRefsOnly )
  {
    ...
    if( lAllDescendantTablesForSingleTable.contains( pTable ) )
    {
      ...
      {
        if( (lAssociation.getTableTo().equals( pTable ) && lAllDescendantTablesForSingleTable.contains( lAssociation.getTableFrom() ))
            || (lAssociation.getTableFrom().equals( pTable ) && lAllDescendantTablesForSingleTable.contains( lAssociation.getTableTo() )) )
        {
